...
- **Auto-update** - Updates are available via GitHub Releases and Homebrew
...
```bash
brew install --cask tamaga3131400/tap/sprachhilfe
```

### Direct Download

Download the latest signed DMG from [GitHub Releases](https://github.com/tamaga3131400/sprachhilfe-dist/releases/latest).

Install the DMG by opening it and dragging Sprachhilfe to your Applications folder.

## Quick Start

1. Install Sprachhilfe from Homebrew or the latest DMG.
...
These steps are for official Sprachhilfe release builds on macOS. They remove the app itself, its local state, widget data, and stored secrets so you can reinstall from a clean slate.

If you installed via Homebrew, you can optionally start with:

```bash
brew uninstall --cask sprachhilfe
```

That removes the app bundle, but it does not reliably remove all files in `~/Library` or Sprachhilfe entries in Keychain.

If `~/Library` is hidden in Finder, use `Go -> Go to Folder...` and paste the paths below.

1. Quit Sprachhilfe if it is running.
2. Delete the app bundle:
   ```bash
   rm -rf /Applications/Sprachhilfe.app
   ```
3. Delete app data and plugins:
   ```bash
   rm -rf ~/Library/Application\ Support/Sprachhilfe
   ```
4. Delete preferences:
   ```bash
   rm -f ~/Library/Preferences/com.sprachhilfe.mac.plist
   ```
5. Delete widget and app group data used by official releases:
   ```bash
   rm -rf ~/Library/Group\ Containers/2D8ALY3LCL.com.sprachhilfe.mac
   ```
6. Remove Sprachhilfe secrets from Keychain:
   - In Keychain Access, search for `com.sprachhilfe.mac.apikey` and delete matching items.
   - This includes API and plugin secrets stored under the `com.sprachhilfe.mac.apikey.*` service prefix.
7. If you installed the CLI tool from Settings > Advanced, remove it too:
   ```bash
   rm -f /usr/local/bin/sprachhilfe
   ```
8. Optional: if you want to remove exported user files as well, delete:
   ```bash
   rm -rf ~/Documents/Sprachhilfe\ Recordings
   ```
9. Restart your Mac, then install the latest build again.

If a fresh install still crashes immediately after these steps, please open an issue and include your macOS version, how you installed Sprachhilfe, and whether the crash happens on first launch or after granting permissions.
...
1. Clone the repository:
   ```bash
   git clone https://github.com/tamaga3131400/sprachhilfe-source.git
   cd sprachhilfe-source
   ```

2. Open in Xcode 16+:
   ```bash
   open Sprachhilfe.xcodeproj
   ```

3. Select the Sprachhilfe scheme and build (Cmd+B). Swift Package dependencies (WhisperKit, FluidAudio, SprachhilfePluginSDK) resolve automatically.
...
Uploads to `/v1/transcribe` are limited to 256 MiB, including stdin uploads from the CLI. Requests above that size return `413 Payload Too Large`. Local CLI file paths use a direct handoff to the running Sprachhilfe app instead of uploading the file bytes.
...
curl -X PUT http://localhost:8978/v1/dictionary/terms \
  ...
  -d '{"terms":["Sprachhilfe","WhisperKit"],"replace":false}'
...
Dictation control records microphone audio for system-wide insertion. A completed dictation session returns text that Sprachhilfe can paste back into the active app.

### Recorder Control

Recorder control uses the same recorder path as the Sprachhilfe UI, including microphone capture, optional system audio capture, mixing, finalization, and final transcription. Use it for automations that need a saved recording file or meeting/system-audio transcription without auto-pasting into another app.
...
- `mic` - `true`, `false`, `1`, or `0`. If omitted, Sprachhilfe uses the current recorder microphone setting.
- `system_audio` - `true`, `false`, `1`, or `0`. If omitted, Sprachhilfe uses the current recorder system-audio setting.
...
```json
{
  ...
  "output_file": "~/Documents/Sprachhilfe Recordings/Recording 2026-05-20 14-30-00.m4a"
}
```
...
Sprachhilfe includes a command-line tool for shell-friendly transcription. It is part of the advanced automation surface and connects to the running local API server.

### Installation

Install via Settings > Advanced > CLI Tool > Install. This places the `sprachhilfe` binary in `/usr/local/bin`.

### Commands

```bash
sprachhilfe status              # Show server status
sprachhilfe models              # List available models
sprachhilfe transcribe file.wav # Transcribe an audio file
```
...
sprachhilfe transcribe recording.wav --language de --json

# Restrict auto-detection to a shortlist
sprachhilfe transcribe recording.wav --language-hint de --language-hint en

# Pipe audio from stdin
cat audio.wav | sprachhilfe transcribe -

# Use in a script
sprachhilfe transcribe meeting.m4a --json | jq -r '.text'
```
...
Local file paths are handed to the running Sprachhilfe app directly, so large files do not need to fit inside an HTTP upload body. Stdin usage (`sprachhilfe transcribe -`) still uses the regular `/v1/transcribe` upload endpoint and is limited to 256 MiB.
...
When you start dictating, Sprachhilfe matches the active app and browser URL against enabled workflows with the following priority:
...
Sprachhilfe supports plugins for adding custom LLM providers, transcription engines, TTS providers, post-processors, and action plugins. Plugins are macOS `.bundle` files placed in `~/Library/Application Support/Sprachhilfe/Plugins/`.
...
See [SprachhilfePluginSDK/Plugins/README.md](SprachhilfePluginSDK/Plugins/README.md) for the full plugin development guide, including the event bus, host services API, and manifest format.

## Architecture

```
Sprachhilfe/
├── sprachhilfe-cli/           # Command-line tool (status, models, transcribe)
...
├── SprachhilfePluginSDK/   # Plugin SDK (Swift package)
│   ├── Plugins/            # First-party plugin sources and manifests
├── SprachhilfeWidgetExtension/ # WidgetKit widgets (stats, activity, history)
├── SprachhilfeWidgetShared/    # Shared widget data models
