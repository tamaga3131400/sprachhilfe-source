...
private let logger = Logger(subsystem: "com.sprachhilfe.mac.dev.widgets", category: "Timeline")

struct SprachhilfeEntry: TimelineEntry {
    let date: Date
    let data: WidgetData
}

struct SprachhilfeTimelineProvider: TimelineProvider {
    func placeholder(in context: Context) -> SprachhilfeEntry {
        SprachhilfeEntry(date: .now, data: .empty)
    }

    func getSnapshot(in context: Context, completion: @escaping (SprachhilfeEntry) -> Void) {
        let data = loadWithLogging()
        completion(SprachhilfeEntry(date: .now, data: data))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<SprachhilfeEntry>) -> Void) {
        let data = loadWithLogging()
        let entry = SprachhilfeEntry(date: .now, data: data)
...
struct StatsWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: SprachhilfeTimelineProvider()) { entry in
...
@main
struct SprachhilfeWidgetBundle: WidgetBundle {
