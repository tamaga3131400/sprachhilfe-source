...
private struct MenuBarExtraLabel: View {
    ...
    private var title: String {
        AppConstants.isDevelopment ? "Sprachhilfe Dev" : "Sprachhilfe"
    }
    ...
    var body: some View {
        Image(nsImage: MenuBarLogoMarkImage.image(
...
enum MenuBarLogoMarkImage {
    ...
    static let idleBarHeights: [CGFloat] = [0.5, 0.75, 1.0, 0.75, 0.5]

    static func image(isRecordingActive: Bool, audioLevel: Float = 0) -> NSImage {
        ...
        if isRecordingActive {
            let level = CGFloat(max(0.15, audioLevel))
            heights = [
                level * 0.5,
                level * 0.75,
                level,
            ...
            heights = idleBarHeights
        }

        for rect in barRects(in: CGRect(origin: .zero, size: size), relativeHeights: heights) {
            NSBezierPath(
                roundedRect: rect,
    ...
    static func barRects(in rect: CGRect, relativeHeights: [CGFloat] = idleBarHeights) -> [CGRect] {
        let side = min(rect.width, rect.height) * 0.875
        let barWidth = side / 7
        let spacing = barWidth / 2
        let totalWidth = (barWidth * CGFloat(relativeHeights.count))
            + (spacing * CGFloat(relativeHeights.count - 1))
        var x = rect.midX - (totalWidth / 2)

        return relativeHeights.map { relativeHeight in
...
struct SprachhilfeApp: App {
    ...
    private var settingsContent: some View {
        if AppConstants.isRunningTests {
            ...
            SettingsView()
                .sheet(item: $startupSheet) { route in
                    switch route {
                    case .welcome:
                        WelcomeSheet()
                    }
    ...
    private func refreshStartupSheet() {
        ...
        if !UserDefaults.standard.bool(forKey: UserDefaultsKeys.welcomeSheetShown) {
            startupSheet = .welcome
        }
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    static func registerDefaultUserDefaults(_ defaults: UserDefaults = .standard) {
        defaults.register(defaults: [
            ...
            UserDefaultsKeys.selectedLanguage: "de",
            UserDefaultsKeys.preferredAppLanguage: "de"
        ])
    }
    ...
    private func isManagedWindow(_ window: NSWindow) -> Bool {
        ...
        return title == String(localized: "Settings")
            || title == String(localized: "Sprachhilfe Setup")
