...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Sprachhilfe", category: "PluginRegistry")
...
final class PluginRegistryService: ObservableObject {
    ...
    nonisolated static func registryFeed(appVersion: String) -> RegistryFeed {
    ...
    nonisolated static func isTrustedRegistryDownloadURL(
        ...
        return components.path.hasPrefix(
            "/tamaga3131400/sprachhilfe-dist/releases/download/"
        )
    }

    init(
        registryBaseURL: URL = URL(string: "https://tamaga3131400.github.io/sprachhilfe")!,
    ...
    func fetchRegistry(force: Bool = false) async -> Bool {
        ...
        fetchState = .loading
        let feed = Self.registryFeed(appVersion: resolvedAppVersion)
