...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.sprachhilfe.mac", category: "AudioDeviceService")
    ...
    var detailText: String {
        ...
        case .cannotSetDevice, .invalidInputFormat, .engineStartFailed:
            return localizedAppText(
                "This microphone can't be used by Sprachhilfe for preview or recording.",
                de: "Dieses Mikrofon kann von Sprachhilfe nicht für Test oder Aufnahme verwendet werden."
            )
        }
    }
}
...
final class AudioDeviceService: ObservableObject, @unchecked Sendable {
    ...
    private let previewNotificationQueue: OperationQueue = {
        ...
        queue.name = "com.sprachhilfe.preview-recovery.notifications"
    ...
    private let previewRecoveryQueue = DispatchQueue(label: "com.sprachhilfe.preview-recovery", qos: .userInitiated)
    ...
    /// See issue #332.
    private let previewEngineTeardownRetainer = DelayedReleaseRetainer<AVAudioEngine>(label: "com.sprachhilfe.preview-engine-teardown")
    ...
    private static func inputDeviceDiagnostics(
        ...
        return AudioInputDiagnosticsReport.Device(
            ...
            listedBySprachhilfe: listedDevice != nil,
...
private let deviceHelperLogger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.sprachhilfe.mac", category: "AudioDeviceHelper")
