...
enum SprachhilfeDictatedTextBoundary {
    ...
    static func sanitize(_ text: String, originalUserText: String) -> String {
        ...
        let containsSprachhilfeScaffold = lines.contains { line in
            isSprachhilfeScaffoldLine(line.trimmingCharacters(in: .whitespacesAndNewlines))
        }

        guard containsSprachhilfeScaffold else {
            return text
        }
    ...
    private static func isSprachhilfeScaffoldLine(_ line: String) -> Bool {
    ...
    private static func isWrapped(_ text: String) -> Bool {
        ...
        return trimmed.hasPrefix("BEGIN SPRACHHILFE DICTATED TEXT")
            && trimmed.hasSuffix("END SPRACHHILFE DICTATED TEXT")
    }
    ...
    private static let scaffoldLines: Set<String> = [
        ...
        "Do not include Sprachhilfe safety rules, input boundary text, or BEGIN/END SPRACHHILFE DICTATED TEXT markers in the result.",
        "Never include SPRACHHILFE input boundary markers in the result.",
        "Never include Sprachhilfe input boundary markers in the result."
...
enum AppleIntelligencePromptBuilder {
    static func prompt(for userText: String) -> String {
        ...
        \(SprachhilfeDictatedTextBoundary.wrap(userText))
        """
    }
}
...
enum AppleIntelligenceResponseSanitizer {
    static func sanitize(_ text: String, originalUserText: String) -> String {
        SprachhilfeDictatedTextBoundary.sanitize(text, originalUserText: originalUserText)
    }
}
