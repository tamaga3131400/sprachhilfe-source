...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.sprachhilfe.mac", category: "AudioRecorderService")
...
final class AudioRecorderService: ObservableObject, @unchecked Sendable {
    ...
    static let recordingsDirectoryName = "Sprachhilfe Recordings"
    ...
    private func startSystemAudioRecording(outputURL: URL) async throws {
        ...
        try stream.addStreamOutput(output, type: .audio, sampleHandlerQueue: DispatchQueue(label: "com.sprachhilfe.system-audio", qos: .userInteractive))
