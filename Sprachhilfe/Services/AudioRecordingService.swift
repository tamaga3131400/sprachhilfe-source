...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.sprachhilfe.mac", category: "AudioRecordingService")
...
final class AudioRecordingService: ObservableObject, @unchecked Sendable {
    private let recoveryNotificationQueue: OperationQueue = {
        ...
        queue.name = "com.sprachhilfe.audio-recovery.notifications"
    ...
    private let processingQueue = DispatchQueue(label: "com.sprachhilfe.audio-processing", qos: .userInteractive)
    private let recoveryQueue = DispatchQueue(label: "com.sprachhilfe.audio-recovery", qos: .userInitiated)
    private let engineTeardownRetainer = DelayedReleaseRetainer<AVAudioEngine>(label: "com.sprachhilfe.audio-engine-teardown")
