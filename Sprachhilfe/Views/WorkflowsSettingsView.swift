    ...
    private var header: some View {
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(
                    localizedAppText(
                        "Create and manage the workflows Sprachhilfe should actively run.",
                        de: "Erstelle und verwalte die Workflows, die Sprachhilfe aktiv ausführen soll."
                    )
                )
                .font(.subheadline)
...
private struct WorkflowEditorPage: View {
    ...
    @State private var isWhisperModeExpanded = false
    @State private var isFineTuningExpanded = false
    @State private var isActionTargetExpanded = false
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if draft.template == .custom {
                    WorkflowTextEditorField(
                        ...
                        autosaveKey: "text-editor.workflow.\(workflow?.id.uuidString ?? "new").instruction"
                    )
                }
                ...
                if draft.usesLLMProcessing {
                    WorkflowCollapsibleSection(
                        ...
                        description: localizedAppText("Optional: add tone, length, or wording hints.", de: "Optional: ergänze Hinweise zu Ton, Länge oder Formulierung."),
                        ...
                        WorkflowTextEditorField(
                            ...
                            autosaveKey: "text-editor.workflow.\(workflow?.id.uuidString ?? "new").finetuning"
                        )
                    }
                }
                ...
                if shouldShowActionTargetSection {
                    WorkflowCollapsibleSection(
                        ...
                        description: localizedAppText("Choose where the workflow result should be sent.", de: "Wähle, wohin das Workflow-Ergebnis gesendet werden soll."),
                        isExpanded: $isActionTargetExpanded
                    ) {
                        actionTargetSection
                    }
                }
                ...
                if let autosaveKey {
                    Button {
                        TextEditorWindowManager.shared.present(
                            ...
                            title: title.isEmpty ? localizedAppText("Edit", de: "Bearbeiten") : title,
                            text: $text
                        )
                    } label: {
...
// MARK: - Workflow Collapsible Section

private struct WorkflowCollapsibleSection<Content: View>: View {
    let title: String
    let description: String?
