...
struct PluginSettingsView: View {
    ...
    @State private var expandedCategories: Set<PluginCategory> = []
    ...
    private var integrationSummaryText: String {
        ...
        if updates > 0 {
            return localizedAppText(
                ...
                de: "\(installed) installiert · \(updates) Updates"
            )
        }
        return localizedAppText(
            "\(installed) installed",
    ...
    private func integrationTabCount(for tab: IntegrationTab) -> Int {
        ...
        case .discover:
            return modelCatalogProviders.count
        }
    }
    ...
    private func integrationTabSubtitle(for tab: IntegrationTab) -> String {
        ...
        case .discover:
            return localizedAppText("\(count) sources", de: "\(count) Quellen")
        }
    }
    ...
    private func inferredCategories(for plugin: LoadedPlugin) -> [PluginCategory] {
        ...
        if plugin.instance is any GraphStoragePlugin { categories.append(.memory) }
        return categories
    }
    ...
    private var installedTab: some View {
        LazyVStack(spacing: 12) {
            if filteredInstalledPlugins.isEmpty {
                ...
                ForEach(PluginCategory.allCases.sorted(by: { $0.sortOrder < $1.sortOrder }), id: \.self) { category in
                    let categoryPlugins = pluginsByCategory(for: category)
            ...
            if !modelCatalogProviders.isEmpty {
    ...
    private func pluginsByCategory(for category: PluginCategory) -> [(plugin: LoadedPlugin, registryPlugin: RegistryPlugin?, hosting: PluginHosting, source: IntegrationPluginSource, installInfo: PluginInstallInfo)] {
        filteredInstalledPlugins.compactMap { plugin in
            let registryPlugin = registryService.registry.first(where: { $0.id == plugin.id })
            let pluginCategories = categoriesForPlugin(plugin, registryPlugin: registryPlugin)
    ...
    private func toggleCategory(_ category: PluginCategory) {
    ...
    // MARK: - Available Tab (Model Discovery)

    private var modelCatalogProviders: [(plugin: LoadedPlugin, provider: any PluginModelCatalogProviding)] {
        pluginManager.loadedPlugins.compactMap { plugin in
            guard let provider = plugin.instance as? any PluginModelCatalogProviding else { return nil }
            ...
            if modelCatalogProviders.isEmpty {
                IntegrationEmptyState(
                    title: localizedAppText(
                        "No plugin currently offers a model catalog to browse.",
                        de: "Kein Plugin bietet aktuell einen durchsuchbaren Modell-Katalog an."
                    ),
                ...
                ForEach(modelCatalogProviders, id: \.plugin.id) { entry in
                    ModelCatalogSection(plugin: entry.plugin, provider: entry.provider) {
                        PluginSettingsWindowManager.shared.present(entry.plugin)
                    }
                }
            }
        }
    ...
    private var installedDiscoverBannerCopy: some View {
        discoverHeroCopy(
            title: localizedAppText("Discover local models", de: "Lokale Modelle entdecken"),
            subtitle: localizedAppText(
                "Browse MLX-compatible models from Hugging Face and add them directly here.",
                de: "Durchsuche MLX-kompatible Modelle von Hugging Face und füge sie direkt hier hinzu."
            ),
            actionTitle: localizedAppText("Discover models", de: "Modelle entdecken"),
    ...
    private func boundaryUpgradeMessage(for plugin: RegistryPlugin, notice: ExternalBundleNotice?) -> String {
        ...
        case .boundaryUpgradeRequired(let installedVersion, let availableVersion):
            return String(
                localized: "Installing \(plugin.name) \(availableVersion) will replace an older external plugin bundle (\(installedVersion)) that was kept for another Sprachhilfe runtime. Older app versions may stop using that bundle after this replacement."
            )
        default:
            return String(
                localized: "Installing this plugin will replace an older external bundle that was kept for another Sprachhilfe runtime. Older app versions may stop using that bundle after this replacement."
            )
        }
    }
}
...
private struct ModelCatalogSection: View {
    let plugin: LoadedPlugin
    let provider: any PluginModelCatalogProviding
    let onManage: () -> Void

    @State private var query = ""
    @State private var results: [PluginCatalogModel] = []
    ...
    var body: some View {
        ...
        .onAppear {
            if results.isEmpty {
                results = provider.cachedModelCatalog
            }
        }
    }
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 10) {
            IntegrationIcon(
                systemName: plugin.manifest.iconSystemName ?? "shippingbox",
                tint: .blue,
                imageURL: nil,
                darkImageURL: nil
            )
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(plugin.manifest.name)
                    .font(.headline)
                Text(provider.modelCatalogSourceDescription)
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Button(action: onManage) {
                Label(localizedAppText("Manage", de: "Verwalten"), systemImage: "gearshape")
            }
            .buttonStyle(.bordered)
            .controlSize(.small)
        }
    ...
    private var searchField: some View {
        HStack(spacing: 10) {
            Image(systemName: "magnifyingglass")
                .foregroundStyle(.secondary)
            TextField(
                localizedAppText("Search models (e.g. llama, qwen, mistral)", de: "Modelle suchen (z. B. llama, qwen, mistral)"),
                text: $query
            )
            ...
            if isSearching {
                ...
                Button(localizedAppText("Search", de: "Suchen")) {
                    search()
                }
                .buttonStyle(.borderless)
                .controlSize(.small)
    ...
    private func search() {
        ...
        hasSearched = true
        Task {
            let found = await provider.searchModelCatalog(query: trimmed)
            results = found
            isSearching = false
        }
    }
    ...
    private var resultsContent: some View {
        if results.isEmpty {
            Text(hasSearched
                 ? localizedAppText("No models found.", de: "Keine Modelle gefunden.")
                 : localizedAppText("Popular models appear here, or search above.", de: "Beliebte Modelle erscheinen hier, oder suche oben."))
                .font(.caption)
                .foregroundStyle(.secondary)
                .padding(.vertical, 8)
        } else {
            LazyVStack(spacing: 8) {
                ForEach(results) { model in
                    ModelCatalogRow(
                        model: model,
                        isAdded: addedIds.contains(model.id) || provider.isCatalogModelAdded(model.id),
                        onAdd: {
                            if provider.addCatalogModel(id: model.id, displayName: model.title) {
                                addedIds.insert(model.id)
                            }
                        }
                    )
                }
    ...
    private var manualAddRow: some View {
        HStack(spacing: 10) {
            TextField(
                localizedAppText("Or paste a Hugging Face repo ID", de: "Oder Hugging-Face-Repo-ID einfügen"),
                text: $manualRepoId
            )
            ...
            Button {
                ...
                guard !trimmed.isEmpty else { return }
                if provider.addCatalogModel(id: trimmed, displayName: trimmed) {
                    addedIds.insert(trimmed)
                    manualRepoId = ""
                }
            } label: {
            ...
            .buttonStyle(.bordered)
            .controlSize(.small)
            .disabled(manualRepoId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
        }
    }
}
...
private struct ModelCatalogRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            if isAdded {
                Label(localizedAppText("Added", de: "Hinzugefügt"), systemImage: "checkmark.circle.fill")
                    .font(.caption.weight(.medium))
                    .foregroundStyle(.secondary)
            } else {
                Button(action: onAdd) {
                    Label(localizedAppText("Add", de: "Hinzufügen"), systemImage: "plus.circle")
                }
                .buttonStyle(.bordered)
                .controlSize(.small)
            }
...
private struct IncompatibleBundleRow: View {
    ...
    private var reasonText: String {
        ...
        case .sdkCompatibility(let expected, let actual):
            ...
            return String(localized: "Missing SDK compatibility metadata for this Sprachhilfe runtime.")
        }
    }
}
...
private extension ExternalBundleNotice {
    var detailText: String {
        ...
        case .legacyBundlePresent(let version):
            return String(localized: "External plugin bundle \(version) was kept for an older Sprachhilfe line.")
...
// MARK: - Plugin Category Section (Collapsible)

private struct PluginCategorySection: View {
    let category: PluginCategory
    let plugins: [(plugin: LoadedPlugin, registryPlugin: RegistryPlugin?, hosting: PluginHosting, source: IntegrationPluginSource, installInfo: PluginInstallInfo)]
    let isExpanded: Bool
    let onToggle: () -> Void
    let onUpdate: (RegistryPlugin) -> Void
    let onRepair: (LoadedPlugin) -> Void
    let onUninstall: (LoadedPlugin) -> Void
                ...
                VStack(spacing: 10) {
                    ForEach(plugins, id: \.plugin.id) { item in
                        InstalledPluginRow(
                            ...
                            categories: [category],
                            source: item.source,
                            installInfo: item.installInfo,
                            installState: nil,
                            externalNotice: nil,
                            hosting: item.hosting,
                            registryPlugin: item.registryPlugin,
                            onUpdate: {
                                if let registryPlugin = item.registryPlugin {
                                    onUpdate(registryPlugin)
                                }
                            },
                            onRepair: { onRepair(item.plugin) },
                            onUninstall: { onUninstall(item.plugin) }
                        )
                        .background {
