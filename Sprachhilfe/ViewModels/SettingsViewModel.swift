...
enum LanguageSelection: Equatable, Sendable {
    ...
    func normalizedForSupportedLanguages(_ supportedLanguages: [String]) -> LanguageSelection {
        ...
        switch self {
        case .exact(let code):
            if let mapped = Self.matchSupportedLanguage(code, supported: supportedLanguages, supportedSet: supportedSet) {
                return .exact(mapped)
            }
            return .auto
        case .hints(let codes):
            let mapped = codes.compactMap {
                Self.matchSupportedLanguage($0, supported: supportedLanguages, supportedSet: supportedSet)
            }
            return withSelectedCodes(mapped, nilBehavior: .auto)
        case .inheritGlobal, .auto:
            return self
        }
    }

    /// Maps a selected language code onto what an engine actually supports, tolerating
    /// region differences (e.g. a regional code like `de-DE` from one engine vs. a base
    /// code like `de` from another). Without this, picking "Deutsch (Deutschland)"
    /// silently falls back to auto-detect on engines that only list `de`.
    private static func matchSupportedLanguage(
        ...
        // 1. Exact match.
        if supportedSet.contains(code) { return code }

        let base = baseLanguageSubtag(code)
        // 2. Selected is regional (de-DE), engine supports the base (de).
        if base != code, supportedSet.contains(base) { return base }
        // 3. Selected is a base (de), engine supports a regional variant (de-DE).
        return supported.first { baseLanguageSubtag($0) == base }
    }

    private static func baseLanguageSubtag(_ code: String) -> String {
...
final class SettingsViewModel: ObservableObject {
    ...
    init(modelManager: ModelManagerService) {
        ...
        self.translationTargetLanguage = UserDefaults.standard.string(forKey: UserDefaultsKeys.translationTargetLanguage) ?? "de"
    }
