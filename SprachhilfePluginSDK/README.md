# Sprachhilfe Plugin SDK

Build plugins for [Sprachhilfe](https://github.com/tamaga3131400/sprachhilfe-source) to add transcription engines, text-to-speech providers, LLM providers, post-processors, and custom actions.

## Quick Start

### 1. Create an Xcode Bundle Target

In your Xcode project (or the Sprachhilfe project itself):

1. **File > New > Target > macOS > Bundle**
2. Set **Product Name** to your plugin name (e.g. `MyPlugin`)
3. Add the `SprachhilfePluginSDK` package as a dependency
...
- `minHostVersion` - Minimum Sprachhilfe version required
...
- **Manual**: Copy the `.bundle` to `~/Library/Application Support/Sprachhilfe/Plugins/`
- **Symlink** (development): `ln -s /path/to/DerivedData/.../MyPlugin.bundle ~/Library/Application\ Support/Sprachhilfe/Plugins/`
...
optionally add `DictionaryTermsBudgetProviding` so Sprachhilfe clips the global
...
`sdkCompatibilityVersion = "v1"` and automatically continue to use Sprachhilfe's
...
`TTSPlaybackSession` must keep track of active playback so Sprachhilfe can stop or replace it:
...
func activate(host: HostServices) {
    ...
    // File storage (~/Library/Application Support/Sprachhilfe/PluginData/<pluginId>/)
...
`availableWorkflows` exposes read-only `PluginWorkflowInfo` snapshots. Plugins can inspect workflow names, templates, enabled state, trigger metadata, behavior settings, LLM provider/model choices, temperature directives, and output routing without depending on Sprachhilfe's internal SwiftData models.
...
| `minHostVersion` | No | Minimum Sprachhilfe version (e.g. `1.0.0`) |
...
| `hosting` | No | Marketplace hosting classification: `local` or `cloud`. If omitted, Sprachhilfe falls back to `requiresAPIKey == true` as cloud and otherwise local. |
...
To distribute via the Sprachhilfe plugin marketplace:
...
2. Submit a PR adding `PluginRegistry/community-v1/com.yourname.myplugin.json`
   on `main`.
3. Keep `releases[]` omitted or empty until a Sprachhilfe maintainer publishes
...
4. After review, a maintainer runs `plugin-release.yml` with
   ...
   publishes the Sprachhilfe-owned ZIP to `gh-pages/plugins-community-v1.json`.

Community marketplace artifacts must be built and hosted by Sprachhilfe.
...
`source` is registry metadata for the Sprachhilfe Integrations UI. Omit it for official marketplace entries; Sprachhilfe treats missing values as `official`. Use `"source": "community"` for community-maintained plugins submitted to the `1.4+` community feed. This field is not required in the plugin bundle manifest.
...
Sprachhilfe maintainer publishes the artifact, release entries are written with
`version`, `minHostVersion`, `sdkCompatibilityVersion`, `minOSVersion`,
`supportedArchitectures`, `size`, and a Sprachhilfe-owned `downloadURL`.
