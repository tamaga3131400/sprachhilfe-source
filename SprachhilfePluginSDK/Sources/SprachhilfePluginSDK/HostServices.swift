...
public enum PluginHTTPClient {
    private static let logger = Logger(subsystem: "com.sprachhilfe.sdk", category: "HTTP")
...
public struct PluginOpenAIChatHelper: Sendable {
    ...
    public func process(
        ...
        // Primary field; reasoning models may leave `content` empty and put the
        // answer in `reasoning_content` instead.
        let content = (message["content"] as? String) ?? ""
        var result = content.trimmingCharacters(in: .whitespacesAndNewlines)
        if result.isEmpty, let reasoning = message["reasoning_content"] as? String {
            result = reasoning.trimmingCharacters(in: .whitespacesAndNewlines)
        }

        if result.isEmpty {
            let finishReason = (first["finish_reason"] as? String) ?? "unknown"
            throw PluginChatError.apiError(
                "Model returned empty content (finish_reason: \(finishReason)). "
                + "If this is a length cut-off, raise Max Output Tokens; for local servers, "
                + "also check the model's context window."
            )
        }
