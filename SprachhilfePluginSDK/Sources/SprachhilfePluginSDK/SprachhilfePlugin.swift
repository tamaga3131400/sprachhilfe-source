...
public protocol SprachhilfePlugin: AnyObject, Sendable {
...
public extension SprachhilfePlugin {
    @MainActor
...
public protocol PluginSettingsActivityReporting: SprachhilfePlugin {
...
public protocol LLMProviderPlugin: SprachhilfePlugin {
    var providerName: String { get }
    var isAvailable: Bool { get }
    var supportedModels: [PluginModelInfo] { get }
    /// True for in-process local models (e.g. MLX/Gemma) whose context length is bounded by
    /// device memory. The host uses this to split very long inputs for these providers only.
...
public protocol AdditionalLLMProvidersProviding: SprachhilfePlugin {
...
public protocol PluginDownloadedModelManaging: SprachhilfePlugin {
    var downloadedModels: [PluginModelInfo] { get }
    func deleteDownloadedModel(_ modelId: String) async throws
}

/// A single searchable/addable entry in a plugin's model catalog (e.g. a Hugging Face repo).
public struct PluginCatalogModel: Identifiable, Sendable, Equatable {
    public let id: String
    ...
    public init(id: String, title: String, subtitle: String? = nil, isRecommended: Bool = false, detailText: String? = nil) {
        self.id = id
        self.title = title
...
/// Optional protocol for plugins that expose a searchable, addable model catalog (e.g. a
/// HuggingFace-backed local model provider). Lets the host build a generic "browse models" UI
/// without knowing the plugin's storage layout or catalog source.
public protocol PluginModelCatalogProviding: SprachhilfePlugin {
    /// Short description of where the catalog is sourced from, e.g. "mlx-community on Hugging Face".
    var modelCatalogSourceDescription: String { get }
    /// Last fetched results, for offline/instant display before a fresh search completes.
    var cachedModelCatalog: [PluginCatalogModel] { get }
...
public protocol PostProcessorPlugin: SprachhilfePlugin {
...
public protocol FileJobAutomationPlugin: SprachhilfePlugin {
...
public protocol DictionaryTermsCapabilityProviding: SprachhilfePlugin {
...
public protocol TranscriptionEnginePlugin: SprachhilfePlugin {
    var providerId: String { get }
    var providerDisplayName: String { get }
...
public protocol AdditionalTranscriptionEnginesProviding: SprachhilfePlugin {
...
public protocol TTSProviderPlugin: SprachhilfePlugin {
    var providerId: String { get }
    var providerDisplayName: String { get }
    var isConfigured: Bool { get }
...
public protocol ActionPlugin: SprachhilfePlugin {
...
public struct MemoryEntry: Codable, Sendable, Identifiable {
    ...
    /// Optional dense embedding for semantic search. Missing in older stored data (decodes
    /// to nil) and for entries created before embeddings were available.
...
public protocol MemoryStoragePlugin: SprachhilfePlugin {
...
// MARK: - Graph Storage (GraphRAG)

public struct GraphNode: Codable, Sendable, Identifiable {
    /// Stable id = normalized name (lowercased, trimmed, diacritic-folded). Dedup key.
    public let id: String
    ...
    public init(id: String? = nil, name: String, type: String, summary: String, metadata: [String: String] = [:]) {
        self.id = id ?? GraphNode.normalizedId(for: name)
        self.name = name
        self.type = type
    ...
    public static func normalizedId(for name: String) -> String {
        name.trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
            .folding(options: [.diacriticInsensitive], locale: Locale(identifier: "de"))
    }
}
...
public struct GraphEdge: Codable, Sendable {
    ...
    public init(from: String, to: String, type: String, summary: String, metadata: [String: String] = [:]) {
        self.from = from
        self.to = to
        self.type = type
        self.summary = summary
...
public struct GraphContext: Sendable {
    ...
    /// Pre-serialized, budget-truncated triple list ready for prompt injection.
    public let promptBlock: String

    public init(nodes: [GraphNode], edges: [GraphEdge], promptBlock: String) {
        self.nodes = nodes
        self.edges = edges
        self.promptBlock = promptBlock
    }

    public var isEmpty: Bool { nodes.isEmpty && edges.isEmpty }
}
...
public struct GraphStats: Sendable {
    ...
    public init(nodeCount: Int, edgeCount: Int) {
        self.nodeCount = nodeCount
        self.edgeCount = edgeCount
    }
}

public protocol GraphStoragePlugin: SprachhilfePlugin {
    var graphName: String { get }
    var isReady: Bool { get }
    /// Idempotent: nodes are deduplicated by normalized id (MERGE/upsert), edges by
    /// (from|type|to). docId is kept as provenance on both so deletion stays coherent.
    func upsert(nodes: [GraphNode], edges: [GraphEdge], docId: String) async throws
    /// Full-text/contains match on names+summaries, restricted to facts sourced from the
    /// supplied document IDs, plus neighbors up to depth 2.
    func retrieveSubgraph(matching: String, allowedDocumentIDs: [String], maxNodes: Int, maxEdges: Int, charBudget: Int) async throws -> GraphContext
    /// Removes the docId provenance; nodes/edges left with no remaining provenance are deleted.
