...
final class AssemblyAIPluginTests: XCTestCase {
    ...
    func testSubmitBodyIncludesSpeakerLabelsOnlyWhenEnabled() {
        ...
        AssemblyAIPlugin.applyDictionaryTerms(prompt: "Sprachhilfe", modelId: "universal-3-pro", to: &disabledBody)
        AssemblyAIPlugin.applyDictionaryTerms(prompt: "Sprachhilfe", modelId: "universal-3-pro", to: &enabledBody)
        XCTAssertEqual(disabledBody["keyterms_prompt"] as? [String], ["Sprachhilfe"])
        XCTAssertEqual(enabledBody["keyterms_prompt"] as? [String], ["Sprachhilfe"])
    }
