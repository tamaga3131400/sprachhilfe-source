...
final class ScriptPlugin: NSObject, PostProcessorPlugin, @unchecked Sendable {
    static let pluginId = "com.sprachhilfe.script"
    ...
    private func runProcess(command: String, input: String, context: PostProcessingContext) async throws -> String {
        try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                // Environment: inherit system + add Sprachhilfe context + ensure UTF-8
                ...
                if let appName = context.appName { env["SPRACHHILFE_APP_NAME"] = appName }
                if let bundleId = context.bundleIdentifier { env["SPRACHHILFE_BUNDLE_ID"] = bundleId }
                if let url = context.url { env["SPRACHHILFE_URL"] = url }
                if let language = context.language { env["SPRACHHILFE_LANGUAGE"] = language }
                if let ruleName = context.ruleName {
                    env["SPRACHHILFE_RULE"] = ruleName
                    env["SPRACHHILFE_PROFILE"] = ruleName
                }
                if let selectedText = context.selectedText { env["SPRACHHILFE_SELECTED_TEXT"] = selectedText }
