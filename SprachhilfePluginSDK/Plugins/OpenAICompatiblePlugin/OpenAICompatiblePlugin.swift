...
struct OpenAICompatibleProfile: Codable, Equatable, Identifiable, Sendable {
    ...
    /// Max output tokens for chat completions. `nil` = unlimited (omit `max_tokens`).
    var maxOutputTokens: Int?

    static let defaultChatRequestTimeout: TimeInterval = 300
    static let minChatRequestTimeout: TimeInterval = 5
    static let maxChatRequestTimeout: TimeInterval = 3600
    static let minMaxOutputTokens = 256
    static let maxMaxOutputTokens = 200_000
...
final class OpenAICompatiblePlugin: NSObject,
    ...
    @unchecked Sendable
{
    static let pluginId = "com.sprachhilfe.openai-compatible"
    ...
    /// Set the max output tokens for a profile. Pass `nil` for unlimited (omits `max_tokens`).
    func setMaxOutputTokens(_ tokens: Int?, for profileId: String) {
        let clamped: Int? = tokens.map {
            min(max($0, OpenAICompatibleProfile.minMaxOutputTokens),
                OpenAICompatibleProfile.maxMaxOutputTokens)
        }
        updateProfile(profileId) { profile in
            profile.maxOutputTokens = clamped
        }
    }
...
private struct OpenAICompatibleSettingsView: View {
    ...
    @State private var maxTokensInput = ""
    ...
    private var maxTokensSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Max Output Tokens", bundle: bundle)
                .font(.headline)

            HStack(spacing: 8) {
                TextField(String(localized: "Unlimited", bundle: bundle), text: $maxTokensInput)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 100)
                    .onSubmit(saveMaxTokens)
                    .accessibilityLabel(Text("Max Output Tokens", bundle: bundle))

                Button(String(localized: "Save", bundle: bundle), action: saveMaxTokens)
                    .buttonStyle(.bordered)
                    .controlSize(.small)
            }

            Text("Maximum number of tokens the model may generate per response. Leave empty for unlimited (the server decides) — recommended so long answers are not cut off. Set a number only if you want a hard cap.", bundle: bundle)
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private func syncFieldsFromSelectedProfile() {
        ...
        maxTokensInput = profile.maxOutputTokens.map(String.init) ?? ""
    ...
    private func saveMaxTokens() {
        guard let selectedProfile else { return }
        let trimmed = maxTokensInput.trimmingCharacters(in: .whitespacesAndNewlines)
        // Empty = unlimited (nil); otherwise must be a positive integer.
        let tokens: Int?
        if trimmed.isEmpty {
            tokens = nil
            ...
            maxTokensInput = selectedProfile.maxOutputTokens.map(String.init) ?? ""
            return
        }
        plugin.setMaxOutputTokens(tokens, for: selectedProfile.id)
        reloadProfiles(selecting: selectedProfile.id, preserveInputs: true)
        if let updated = plugin.profileSnapshot(for: selectedProfile.id) {
            maxTokensInput = updated.maxOutputTokens.map(String.init) ?? ""
        }
    }
