...
final class OpenAIPluginTests: XCTestCase {
    ...
    func testOpenAIAPIKeyModeLoadsOnlyAPIKeyAtActivation() throws {
        let host = try PluginTestHostServices(
            defaults: ["authMode": "api-key"],
            secrets: [
                "api-key": "sk-live",
                "oauth-access-token": "access-token",
                "oauth-refresh-token": "refresh-token",
                "oauth-id-token": "id-token",
            ]
        )
        let plugin = OpenAIPlugin()
        plugin.activate(host: host)

        XCTAssertEqual(host.loadedSecretKeys, ["api-key"])
        XCTAssertTrue(plugin.authStatus(for: .llm).isAvailable)
        XCTAssertEqual(host.loadedSecretKeys, ["api-key"])
    }

    func testOpenAIChatGPTModeLoadsOnlyOAuthCredentialsAtActivation() throws {
        let host = try PluginTestHostServices(
            defaults: ["authMode": "chatgpt"],
            secrets: [
                "api-key": "sk-live",
        ...
        XCTAssertEqual(host.loadedSecretKeys, [
            "oauth-access-token",
            "oauth-refresh-token",
            "oauth-id-token",
        ])
    ...
    func testOpenAIAuthenticationModeSwitchLoadsCredentialsOncePerSession() throws {
        let host = try PluginTestHostServices(
            secrets: [
                "api-key": "sk-live",
                "oauth-access-token": "access-token",
        ...
        XCTAssertEqual(host.userDefault(forKey: "authMode") as? String, "chatgpt")
    ...
    func testOpenAIChatGPTAudioStatusLoadsAPIKeyOnlyOnceWhenNeeded() throws {
        let host = try PluginTestHostServices(
            defaults: ["authMode": "chatgpt"],
            secrets: [
                "api-key": "sk-live",
