...
private final class OpenAILoopbackOAuthServer: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.sprachhilfe.openai.oauth")
    ...
    <html>
      ...
      <body>
        <div class="card">
          ...
          <p>Sie können dieses Fenster schließen und zu Sprachhilfe zurückkehren.</p>
        </div>
...
final class OpenAIPlugin: NSObject,
    ...
    @unchecked Sendable
{
    static let pluginId = "com.sprachhilfe.openai"
    ...
    private var hasLoadedOAuthCredentials = false
    ...
    func activate(host: HostServices) {
        ...
        hasLoadedOAuthCredentials = false
        _authMode = .apiKey
    ...
    private func loadOAuthCredentialsIfNeeded() {
        guard !hasLoadedOAuthCredentials else { return }
        hasLoadedOAuthCredentials = true
...
private struct OpenAISettingsView: View {
    ...
    private func startBrowserLogin() {
        oauthBusy = true
        oauthStatusMessage = String(localized: "Complete the OpenAI login in your browser. Sprachhilfe will finish the connection automatically.", bundle: bundle)
        oauthErrorMessage = nil
