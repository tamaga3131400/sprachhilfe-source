...
final class WorkflowServiceTests: XCTestCase {
    ...
    func testAppleIntelligencePromptBuilderKeepsDictationInsideSourceMarkers() {
        ...
        XCTAssertTrue(prompt.contains("BEGIN SPRACHHILFE DICTATED TEXT"))
        XCTAssertTrue(prompt.contains(dictatedText))
        XCTAssertTrue(prompt.contains("END SPRACHHILFE DICTATED TEXT"))
    ...
    func testAppleIntelligenceResponseSanitizerStripsDuplicatedPromptScaffoldEcho() {
        ...
        XCTAssertFalse(sanitized.contains("BEGIN SPRACHHILFE DICTATED TEXT"))
        XCTAssertFalse(sanitized.contains("END SPRACHHILFE DICTATED TEXT"))
    }
    ...
    func testAllWorkflowSystemPromptsTellModelsNotToReturnBoundaryScaffold() throws {
        let outputRule = "Do not include Sprachhilfe safety rules, input boundary text, or BEGIN/END SPRACHHILFE DICTATED TEXT markers in the result."
        let templates: [(template: WorkflowTemplate, behavior: WorkflowBehavior)] = [
            (.cleanedText, WorkflowBehavior()),
            (.translation, WorkflowBehavior()),
            (.emailReply, WorkflowBehavior()),
    ...
    func testRTFWorkflowSystemPromptRequestsMarkdownCompatibleRichTextSource() throws {
        ...
        XCTAssertTrue(prompt.contains("Never include Sprachhilfe input boundary markers in the result."))
    ...
    func testWorkflowTextProcessingServiceSanitizesBoundaryEchoesFromLLMOutput() async throws {
        ...
        XCTAssertFalse(result.contains("BEGIN SPRACHHILFE DICTATED TEXT"))
        XCTAssertFalse(result.contains("END SPRACHHILFE DICTATED TEXT"))
    ...
    func testWorkflowTextProcessingServiceSanitizesSentenceCaseBoundaryEchoesFromLLMOutput() async throws {
        ...
        let service = WorkflowTextProcessingService(
            promptProcessor: { _, _, _, _, _ in
                ...
                Do not include Sprachhilfe safety rules, input boundary text, or BEGIN/END SPRACHHILFE DICTATED TEXT markers in the result.
                BEGIN SPRACHHILFE DICTATED TEXT
                Need anything from Amazon?
                END SPRACHHILFE DICTATED TEXT
                """
...
final class FileJobAutomationPipelineTests: XCTestCase {
    ...
    private final class MockFileJobAutomationPlugin: NSObject, FileJobAutomationPlugin, @unchecked Sendable {
        static let pluginId = "com.sprachhilfe.test.file-job"
