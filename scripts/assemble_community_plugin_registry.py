#!/usr/bin/env python3
"""Validate and assemble the Sprachhilfe community plugin registry."""
...
SPRACHHILFE_RELEASE_NETLOC = "github.com"
SPRACHHILFE_RELEASE_PATH_PREFIX = "/tamaga3131400/sprachhilfe-dist/releases/download/"
...
def is_sprachhilfe_release_asset_url(value: object) -> bool:
    if not is_non_empty_string(value):
        return False
    ...
    return (
        parsed.scheme == "https"
        and parsed.netloc.lower() == SPRACHHILFE_RELEASE_NETLOC
        and parsed.path.startswith(SPRACHHILFE_RELEASE_PATH_PREFIX)
    )


def validate_sprachhilfe_release_asset_url(
    ...
    if not is_sprachhilfe_release_asset_url(value):
        return [
            f"{filename}: '{field}' must point to a Sprachhilfe-owned GitHub "
            "Release asset under tamaga3131400/sprachhilfe-dist"
        ]
    return []
...
def validate_release(release: object, filename: str, index: int) -> list[str]:
    ...
    errors.extend(
        validate_sprachhilfe_release_asset_url(
